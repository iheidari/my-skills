...
- **Source code / project**: scan for private-API usage, background modes, entitlements, IAP/StoreKit usage, tracking SDKs, hardcoded external-payment links, missing account deletion, and **permission-request flows** — both whether a custom pre-permission dialog can be cancelled before the system prompt, and whether the system prompt is **reachable at all** on a reviewer's device (see the Permission-flow audit steps below).
...
4. **Run the mechanical-pattern sweep (when source is available).** Beyond reasoning about guidelines, there is a set of **objective, code-detectable patterns that get caught every time** — either by Apple's own automated pre-review analysis, or by the reviewer's very first launch. These produce the most predictable rejections, so check them explicitly and default them to **Likely rejection** (not "At risk") when the pattern is present. Grep the project and trace each hit:
   ...
   - **Permission-prompt reachability (2.1) — the counterpart to priming, and the one a code read alone almost never catches.** Priming asks whether the user can *avoid* the prompt. This asks whether the prompt **ever presents at all** on the reviewer's device. Apple rejects it under **2.1 Information Needed**, not under 5.1.x, with wording like: *"The app uses the AppTrackingTransparency framework, but we are unable to locate the App Tracking Transparency permission request when reviewed on iPadOS 26.6."* The call site exists, the tests pass, and nothing was ever asked. **A grep hit proving the request is in the code is not a Pass** — take every request call found in the priming sweep and trace **forwards** from app launch:

     **(a) Anything the request waits on is a gate, and every gate is a way to never ask → Likely rejection.** List what stands between process start and the request, and rule on each. Recurring offenders:
     - **A network round trip.** Consent SDKs are the classic: Google UMP's `AdsConsent.requestInfoUpdate()` / `requestConsentInfoUpdate` has no built-in timeout and **hangs rather than rejects** on a blocked, proxied or throttled network — which describes plenty of review environments. Also remote config, feature flags, A/B assignment, and any "fetch settings then decide" launch step.
     - **Entitlement / subscription state.** `if (!isPro) show the prompt` never asks for a reviewer whose account resolves as subscribed, and asks *late* for anyone whose store lookup is slow. Tier is not a reason to skip a privacy prompt — mount it for everyone and let it no-op where already answered.
     - **Asset or data loading.** An early `if (!fontsLoaded) return null`, a splash screen that owns the first N seconds, a hydration/migration step, a rates/inventory fetch. Each is a plausible place for a launch to end without the ask.
     - **User navigation.** A prompt behind onboarding, a tab the reviewer may not open, a login, or a screen reached only by a specific action. If Apple must see it, it cannot be optional.

     The compliant shape is the inverse: **the ask is mounted unconditionally at launch and everything else is downstream of it.** Where a wait is unavoidable, it must carry its own timeout so a hung dependency degrades into asking anyway.

     **(b) Presentation preconditions — the request can "succeed" having shown nothing → Likely rejection.** iOS declines to present the ATT alert while the app is not foreground-`active`, or while a view-controller transition is in flight, and `ATTrackingManager.requestTrackingAuthorization` / `requestTrackingPermissionsAsync` **resolves with the current status either way**. There is no error, no `false`, no callback saying "not shown". Flag:
     - **The same-tick race.** Hiding a custom pre-prompt and calling the request in the same tick races that modal's dismissal animation. The fix is to sequence on the dismissal actually completing — `Modal`'s `onDismiss`, `dismiss(animated:completion:)` — **plus a timer fallback**, since `onDismiss` is iOS-only and has a history of not firing.
     - **Asking off an inactive app.** A request fired from a splash, a launch task, a push handler, or a `didFinishLaunching` path can land while the app is not yet `active`. Require an explicit check (`AppState.currentState === "active"` / `UIApplication.shared.applicationState == .active`) with a wait, re-checked on every attempt.
     - **No detection of the silent failure.** A status still `undetermined` *after* the call is the only evidence iOS gives that the alert never appeared. Code that ignores that value cannot tell success from silence. Compliant handling re-attempts, and on exhaustion **restores the visible pre-prompt** so the user can trigger it — falling through quietly *is* the rejection state.

     These are timing bugs, so they **reproduce per device**: a real 2026 rejection (Fexi 2.0.2) presented correctly on iPhone and never on iPad, which is exactly why Apple's message named an iPad. "It works on my phone" is not evidence, and neither is "it worked on the last build".

     **(c) Say plainly that no automated gate here catches it.** Unit tests mock the permission module and assert the function was *called* — which passes whether or not iOS drew anything. Type-checks, bundle/export smoke tests and launch smoke tests all stay green. Only a **fresh install on a physical device** distinguishes asked from not-asked. So pair every finding here with the manual check that would have caught it, and recommend it as a pre-submit step regardless of the verdict: **cold launch a fresh install on an iPad as well as an iPhone, on a slow or partially blocked network, with any subscription/entitlement the reviewer's account might have** — the four conditions that separately produced the failures above. As with purpose strings, the fix is **binary**: a new build and build number, not a Resolution Center reply or a metadata edit.

     **(d) Check the reviewer artifact, too.** With this rejection Apple asks for a screen recording captured on a **physical device** showing (i) launch from a fresh install or after resetting tracking permissions, (ii) the permission request appearing before any tracking data is collected, and (iii) the flow that follows — placed in **App Review Information → Notes**. For any app that links ATT, an absent recording is **At risk** on its own: it costs one line of metadata and converts a possible review round-trip into a pre-empted one. Note when advising this that the ATT answer is **once per install and is not reset by deleting the app** — on device it's Settings → Privacy & Security → Tracking, and on a simulator it takes an erase (there is no `simctl privacy` service for tracking), so a "fresh install" recording needs that step first.
   ...
   - **ATT / tracking (5.1.2).** Grep for tracking/ads/analytics SDKs (Facebook, AppsFlyer, Adjust, Branch, Firebase Analytics, AdMob, Segment, Amplitude) and IDFA use. If any track users across apps/sites and there's no `AppTrackingTransparency` prompt → Likely rejection; also flag a privacy-label mismatch. **Finding the prompt in the code closes 5.1.2 only** — Apple's ATT complaint arrives as a 2.1 far more often than as a 5.1.2, so always follow it with the reachability pass above. Likewise check the tracking SDK is initialized *behind* the ATT answer, never before it: collecting first and asking after is its own violation, and reviewers do watch for network calls that precede the prompt.
   ...
   **And the converse: presence of the *required* code is not a Pass either.** For anything the reviewer has to physically witness — a permission prompt above all — the question is not "is it implemented" but "does it run, on a cold launch, on their device, under their network". A call site is evidence it *can* happen. Trace what has to go right first, and rule on that instead.
